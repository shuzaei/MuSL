...
func CopyAgent(a *Agent) *Agent {
	...
	copy(roleCopy, a.role)

	// メモリ系のスライスをディープコピー
	memoryCopy := make([]*Song, 0)
	listenerMemoryCopy := make([]*Song, 0)
	incomingSongsCopy := make([]*Song, 0)
	songEventsCopy := make([]*Event, 0)
	createdEventsCopy := make([]*Event, 0)

	// memory_cスライスのディープコピー
	for _, song := range a.creator.memory {
		memoryCopy = append(memoryCopy, song.DeepCopy()) // Songをディープコピー
	}

	// listener.memoryスライスのディープコピー
	for _, song := range a.listener.memory {
		listenerMemoryCopy = append(listenerMemoryCopy, song.DeepCopy()) // Songをディープコピー
	}

	// listener.incoming_songsスライスのディープコピー
	for _, song := range a.listener.incoming_songs {
		incomingSongsCopy = append(incomingSongsCopy, song.DeepCopy()) // Songをディープコピー
	}

	// listener.song_eventsスライスのディープコピー
	for _, event := range a.listener.song_events {
		songEventsCopy = append(songEventsCopy, event)
	}

	// organizer.created_eventsスライスのディープコピー
	for _, event := range a.organizer.created_events {
		createdEventsCopy = append(createdEventsCopy, event)
	}

	return MakeNewAgent(
		a.id,
		roleCopy, // 参照渡しではなくディープコピーを使用
		a.energy,
		a.default_energy,
